...
执行完后会在当前目录下载两个模型yolov6p5_n.onnx和yolov6p5_n.q.onnx。

yolov6p5_n.onnx为FP32模型。

yolov6p5_n.q.onnx为INT8模型。
...
```
输入：三通道图像路径
输出：输出有三个;outputs[1]维度为(number_dets,4),0-3为未还原的左上角，右下角坐标;outputs[2]维度为(number_dets)，保存的是类别得分值;outputs[3]维度为(number_dets)，保存的是预测的类别标签值。
```
...
```shell
cd python
python test_yolov6.py

# 参数说明
#--model 模型具体路径(默认为../model/yolov6p5_n.q.onnx)  
...
cmake ..
make -j8
./yolov6_demo --model onnx模型路径 --image 图片路径
