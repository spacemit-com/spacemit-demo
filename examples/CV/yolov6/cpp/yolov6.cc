...
// Draw results
void DrawResults(cv::Mat& image, const std::vector<std::vector<float>>& dets, float* scores, int* labels_pred, const std::vector<std::string>& labels, float conf_threshold) {    
...
cv::Mat Yolov6Inference(cv::Mat& image, const std::string& modelPath, const std::string& labelFilePath, float conf_threshold) {
    // Load labels
    std::vector<std::string> labels = ReadLabels(labelFilePath);

    // Initialize ONNX runtime environment and session
    Ort::Env env(ORT_LOGGING_LEVEL_WARNING, "YOLOv6Inference");
    ...
    float* dets_data = outputs[1].GetTensorMutableData<float>();
    float* scores_data = outputs[2].GetTensorMutableData<float>();
    int* labels_pred_data = outputs[3].GetTensorMutableData<int>();

    auto dets_tensor_info = outputs[1].GetTensorTypeAndShapeInfo();
    ...
    // Draw results
    DrawResults(image, dets, scores_data, labels_pred_data, labels, conf_threshold);
