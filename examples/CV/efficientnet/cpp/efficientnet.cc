...
std::vector<float> preprocess(const cv::Mat& img, int resize_width, int resize_height, int crop_width, int crop_height) {
    ...
    int y0 = (h - crop_height) / 2;
    int x0 = (w - crop_width) / 2;


    cv::Rect roi(x0, y0, crop_width, crop_height);
    processed_img = processed_img(roi);
...
std::string  EfficinetNetInference(const std::string& model_path, cv::Mat& image) {
    // Load labels
    ...
    Ort::Env env(ORT_LOGGING_LEVEL_WARNING, "efficientnet");
    ...
    // Run preprocess
    std::vector<float> inputTensor = preprocess(image, 256, 256, inputWidth, inputHeight);
    // Create input tensor
