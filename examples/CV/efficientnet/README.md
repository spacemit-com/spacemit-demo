...
执行完后会在当前目录下载两个模型efficientnet_v1_b1.onnx和efficientnet_v1_b1.q.onnx。

efficientnet_v1_b1.onnx为FP32模型。

efficientnet_v1_b1.q.onnx为INT8模型。
...
# 参数说明
#--model 模型具体路径(默认为../model/efficientnet_v1_b1.q.onnx)  
...
cmake ..
make -j8
./efficientnet_demo --model onnx模型路径 --image 图片路径
# 注意efficientnet.cc中label_file_path的路径是否正确，默认是../../data/label.txt(要以build目录为基准路径)
```
