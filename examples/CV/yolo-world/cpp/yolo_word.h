#ifndef YOLO_WORLD__HPP
#define YOLO_WORLD__HPP
#include <string>
#include <vector>
...
class YOLO_WORLD {
public:
    YOLO_WORLD(const std::string& yolo_world_model_path);
    ~YOLO_WORLD();
    void init_yolo_word_model(std::vector<std::vector<float>>clipdata,const cv::Mat& image,float conf_threshold, float iou_threshold);
    cv::Mat inference_yolo_world_model(cv::Mat& image);
