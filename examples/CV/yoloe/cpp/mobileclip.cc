...
MOBILECLIP::MOBILECLIP(const std::string& text_model_path)
...
{  
    ...
    tokenizer=new MOBILECLIPTokenizer();
    sequence_len=77;
}
...
Ort::Value MOBILECLIP::create_input_tensor(const std::vector<int64_t>& data, const std::vector<int64_t>& shape) {
...
template<typename T>
std::pair<T*, std::vector<int64_t>> MOBILECLIP::get_tensor_data(Ort::Value& tensor) 
{
...
void MOBILECLIP::inference_clip_model(std::vector<std::string> input_texts,int text_num)
{
    clip_data_.clear();
    ...
    for (int i = 0; i < text_num; i++)
    {
        ...
        for (auto x : input) {
            ...
            //std::cout<<static_cast<int64_t>(x)<<" ";
        }
        //std::cout<<"*******"<<std::endl;
