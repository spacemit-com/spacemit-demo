...
执行完后会在当前目录下载两个模型yolov11n_320x320.onnx和yolov11n_320x320.q.onnx。

yolov11n_320x320.onnx为FP32模型。

yolov11n_320x320.q.onnx为INT8模型。
...
# 参数说明
#--model 模型具体路径(默认为../model/yolov11n_320x320.q.onnx)  
...
cmake ..
make -j8
./yolov11_demo --model onnx模型路径 --image 图片路径
