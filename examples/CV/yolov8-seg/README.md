...
下载模型保存为yolov8n-seg.onnx和yolov8n_seg_320.q.onnx。
...
(1,116,2100)中0-3为未还原的左上角，右下角坐标，4-83为每个类的得分值；84-115为预测的掩码系数。
...
<center>
    ...
    color: #999;
    padding: 2px;"> 图1 官方onnx模型输出(模型输入320*320) </div>
</center>
...
([1,64,40,40],[1,80,40,40],[1,1,40,40])代表在一个检测头上的为经过dfl的框坐标，框的每个类的概率，框所有类概率总和。
...
<center>
    ...
    display: inline-block;
    color: #999;
    padding: 2px;"> 图2 非官方onnx模型输出（模型输入320*320) </div>
</center>
...
```shell
python test_yolov8-seg.py

# 参数说明
#--model 模型具体路径(默认为../model/yolov8n_seg_320.q.onnx)  
...
cmake ..
make -j8
./yolov8-seg_demo --model onnx模型路径 --image 图片路径
