...
执行完后会在当前目录下载两个模型swin-tiny_16xb64_in1k.onnx和swin-tiny_16xb64_in1k.q.onnx。

swin-tiny_16xb64_in1k.onnx为FP32模型。

swin-tiny_16xb64_in1k.q.onnx为INT8模型。
...
```shell
python test_swin-tiny_16xb64_in1k.py

# 参数说明
#--model 模型具体路径(默认为../model/swin-tiny_16xb64_in1k.q.onnx)  
...
cmake ..
make -j8
./swin-tiny_demo --model onnx模型路径 --image 图片路径
# 注意swin-tiny.cc中label_file_path的路径是否正确，默认是../../data/label.txt(要以build目录为基准路径)
```
