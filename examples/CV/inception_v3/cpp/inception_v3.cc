...
std::string  Inceptionv3Inference(const std::string& model_path, cv::Mat& image) {
    // Load labels
    ...
    Ort::Env env(ORT_LOGGING_LEVEL_WARNING, "inception_v3");
    ...
    // Run preprocess
    std::vector<float> inputTensor = preprocess(image, 342, 342,inputWidth, inputHeight);
    // Create input tensor
