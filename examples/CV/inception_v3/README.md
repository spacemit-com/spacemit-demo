...
执行完后会在当前目录下载两个模型inception_v3.onnx和inception_v3.q.onnx。

inception_v3.onnx为FP32模型。

inception_v3.q.onnx为INT8模型。
...
# 参数说明
#--model 模型具体路径(默认为../model/inception_v3.q.onnx)  
...
cmake ..
make -j8
./inceptionv3_demo --model onnx模型路径 --image 图片路径
# 注意inception_v3.cc中label_file_path的路径是否正确，默认是../../data/label.txt(要以build目录为基准路径)
```
