...
执行完后会在当前目录下载两个模型inception_v1.onnx和inception_v1.q.onnx。

inception_v1.onnx为FP32模型。

inception_v1.q.onnx为INT8模型。
...
```shell
cd python
python test_inception_v1.py

# 参数说明
#--model 模型具体路径(默认为../model/inception_v1.q.onnx)  
...
cmake ..
make -j8
./inceptionv1_demo --model onnx模型路径 --image 图片路径
# 注意inception_v1.cc中label_file_path的路径是否正确，默认是../../data/label.txt(要以build目录为基准路径)
```
